  ...
  days: [
    {
      ...
      title: "Stan domyślny",
      knowledge: [
        "Przypomnij sobie moment tuż po przebudzeniu, zanim ręka sięgnie po telefon. Głowa sama z siebie już coś robi: przelatuje po wczorajszej rozmowie, planuje dzień, wraca do sprawy sprzed tygodnia. To nie jest szum. To stan domyślny mózgu (ang. default mode network), tryb, w który wchodzi zawsze, kiedy nie dostaje zadania z zewnątrz. W tym trybie mózg porządkuje wspomnienia i planuje przyszłość. Arthur Brooks, profesor Harvardu, który latami bada, co robi ludzi szczęśliwymi, podkreśla, że tylko w tym trybie mózg zadaje sobie pytania o to, co ważne. Nuda to nie przestój. To czas, w którym mózg pracuje nad twoimi sprawami, a nie nad tym, co właśnie podsunął mu ekran.",
